...
Currently we support Multinomial, Dawid and Skene and Hierachical Dawid and Skene models
in the package. Models are specifed as a function with the name of the model you woudld 
like to fit. 

```{r}
# fit the model using MCMC via Stan 
fit <- rater(anesthesia, dawid_skene(), iter = 500, chains = 1)
```

You will see the normal progress output you get whenever **Stan** is sampling.
Note that all models are compiled when the package is installed, rather than at runtime, meaning that you will not need to wait for a model to be compiled when
running analyses.[^1]
...
```{r, eval = FALSE}
fit <- rater(anesthesia, dawid_skene(alpha = c(4, 4, 4, 4)))
```
